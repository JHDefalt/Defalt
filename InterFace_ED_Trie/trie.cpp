...
void trie::on_pushButton_clicked()//查询操作
{
    ...
    else
    {
        ...
        if(trie->checkWord())
        {
            ...
            {
                QString translate;
                string nows = trie->ans->translate;
                int pos = 0;
                int start = 0;
                while (pos != nows.npos) {
                    pos = nows.find('@', start);
...
void trie::on_pushButton_2_clicked()//清空按钮
{
    if(ui->lineEdit->text() == "") QMessageBox::critical(this,"错误","请先输入内容！");
    else ui->lineEdit->setText(""),ui->textBrowser->setText("");
}

void trie::on_actionInsert_triggered()//插入按钮
{
    login *ret = new login();
    ret->setCall(1);
    ret->Show();
...
void trie::on_actionDelete_triggered()//删除按钮
{
    login *ret = new login();
    ret->setCall(2);
    ret->Show();
...
void trie::on_actionModify_triggered()//修改按钮
{
    login *ret = new login();
    ret->setCall(3);
    ret->Show();
...
void trie::on_actionc_triggered()//帮助按钮
{
    new help();
}

void trie::on_actionasd_triggered()//打开词典按钮
{
    login *ret = new login();
    ret->setCall(4);
    ret->Show();
...
void trie::on_actionasd_2_triggered()//关于按钮
{
    new about();
}

void trie::on_actionyu_triggered()//作者邮箱按钮
{
...
void trie::on_actiona_triggered()//打开源码按钮
{
    login *ret = new login();
    ret->setCall(5);
    ret->Show();
...
void trie::on_actiona_3_triggered()//作者qq按钮
{
...
void trie::on_actiona_4_triggered()//作者电话按钮
{
...
void trie::keyPressEvent(QKeyEvent *event)//键盘绑定按钮
{
